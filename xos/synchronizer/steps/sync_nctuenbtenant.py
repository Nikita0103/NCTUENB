...
#from services.nctuenb.models import NCTUENBService, NCTUENBTenant
...
class SyncNCTUENBTenant(SyncInstanceUsingAnsible):

    # Used by XOSObserver : sync_steps to determine dependencies.
    provides = [NCTUENBTenant]

    # The Tenant that is synchronized.
    observes = NCTUENBTenant

    requested_interval = 0

    # Name of the ansible playbook to run.
    template_name = "nctuenbtenant_playbook.yaml"

    # Path to the SSH key used by Ansible.
    service_key_name = "/opt/xos/configurations/mcord/mcord_private_key"

    def __init__(self, *args, **kwargs):
        super(SyncNCTUENBTenant, self).__init__(*args, **kwargs)
    ...
    def get_information(self, o):
        ...
        collect_network = [
           {'name': 'ENB_PRIVATE_IP', 'net_name': 'nctuenb_network'}
        ]
